				 ...
				 enum snd_soc_bias_level level)
{
	...
	//printk(KERN_INFO "####### wm8903_set_bias_level level:%X\n", level);
	switch (level) {
	...
	case SND_SOC_BIAS_STANDBY:
		...
		//snd_soc_write(codec, WM8903_CLOCK_RATES_2, WM8903_CLK_SYS_ENA|WM8903_CLK_DSP_ENA);
		//#if 0
		reg = snd_soc_read(codec, WM8903_VMID_CONTROL_0);
		reg &= ~(WM8903_VMID_RES_MASK);
...
static int wm8903_digital_mute(struct snd_soc_dai *codec_dai, int mute)
{
	...
	//printk(KERN_INFO "####### wm8903_digital_mute mute:%d\n", mute);
			  ...
			  struct snd_soc_dai *dai)
{
	...
	//printk(KERN_INFO "####### wm8903_startup substream:%d\n", substream->stream);
			    ...
			    struct snd_soc_dai *dai)
{
	...
	//printk(KERN_INFO "####### wm8903_shutdown substream:%d\n", substream->stream);
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		wm8903->playback_active--;
	else
		wm8903->capture_active--;
			    ...
			    struct snd_soc_dai *dai)
{
	...
	//printk(KERN_INFO "####### wm8903_hw_params substream:%d\n", substream->stream);
