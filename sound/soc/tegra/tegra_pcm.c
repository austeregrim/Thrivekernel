...
static void tegra_pcm_work(struct work_struct *work)
{
	struct tegra_runtime_data *prtd =
		container_of(work, struct tegra_runtime_data, work);

	//printk(KERN_INFO "############### tegra_pcm_work state:%s func:%s\n", prtd->state==STATE_INIT?"True":"False",
	//		prtd->substream->stream?"Capture":"Playback");

	if(prtd->state == STATE_INIT) {
		tegra_pcm_control(true, prtd->substream->stream);
	} else if(prtd->state == STATE_ABORT || prtd->state == STATE_EXIT) {
		tegra_pcm_control(false, prtd->substream->stream);
	}
}
...
{
	//printk(KERN_INFO "####### tegra_pcm_hw_params type:%d\n", substream->stream);
...
static int tegra_pcm_hw_free(struct snd_pcm_substream *substream)
{
	...
	//printk(KERN_INFO "####### tegra_pcm_hw_free type:%d\n", substream->stream);
	flush_work(&prtd->work);
...
static int tegra_pcm_prepare(struct snd_pcm_substream *substream)
{
	...
	//printk(KERN_INFO "####### tegra_pcm_prepare type:%d\n", substream->stream);
	prtd->dma_pos = 0;
	prtd->period_index = 0;
...
static int tegra_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	//printk(KERN_INFO "####### tegra_pcm_trigger type:%d cmd:%d\n", substream->stream, cmd);
	switch (cmd) {
...
static int tegra_pcm_open(struct snd_pcm_substream *substream)
{
	...
	INIT_WORK(&prtd->work, tegra_pcm_work);
