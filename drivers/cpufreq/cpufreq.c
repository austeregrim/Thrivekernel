...
#define FREQCOUNT 14
#define CPUMVMAX 1500
#define CPUMVMIN 725
int cpufrequency[FREQCOUNT] = {1592000, 1544000, 1472000, 1424000, 1336000, 1232000,1000000, 912000, 816000, 760000, 608000, 456000, 312000, 216000 };
int cpuvoltage[FREQCOUNT] = { 1475, 1475, 1350, 1325, 1300, 1225, 1100, 1050, 1000, 975, 900, 800, 750, 725 };
...
static ssize_t store_UV_mV_table(struct cpufreq_policy *policy, char *buf, size_t count)
{
	...
	unsigned int ret = sscanf(buf, "%d %d %d %d %d %d %d %d %d %d %d %d %d %d", &tmptable[0], &tmptable[1], &tmptable[2], &tmptable[3], &tmptable[4], &tmptable[5], &tmptable[6], &tmptable[7], &tmptable[8], &tmptable[9], &tmptable[10], &tmptable[11], &tmptable[12], &tmptable[13]);
	if (ret != FREQCOUNT)
		return -EINVAL;
	for (i = 0; i < FREQCOUNT; i++)
	{
