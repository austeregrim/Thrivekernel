...
#define CL_SEL_POS	1
#define RTC_RSVDC00 BIT(0)  /* RTC alarm2 detection exits sleep */
...
static int tps6586x_rtc_alarm_poweron(struct device *dev, int enable)
{
  struct device *tps_dev = to_tps6586x_dev(dev);
  int err;

  if (enable)
    err = tps6586x_set_bits(tps_dev, RTC_CTRL, RTC_RSVDC00);
  else
    err = tps6586x_clr_bits(tps_dev, RTC_CTRL, RTC_RSVDC00);
...
static unsigned long saved_alarm2_sec = 0;
static int tps6586x_alarm2_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
  struct tps6586x_rtc *rtc = dev_get_drvdata(dev);
  struct device *tps_dev = to_tps6586x_dev(dev);
  unsigned long seconds;
  unsigned long ticks;
  unsigned long alarm2_count;
  u8 buff[2];
  int err;
  ...
  if (alrm->enabled) {
    tps6586x_rtc_alarm_poweron(dev, 1);
  } else {
    tps6586x_rtc_alarm_poweron(dev, 0);
  }

  saved_alarm2_sec = seconds;

  seconds -= rtc->epoch_start;
  ticks = (unsigned long long)seconds << 10;

  alarm2_count = ticks & 0xffff000;
  alarm2_count = alarm2_count >> 12;

  buff[0] = (alarm2_count >> 8) & 0xff;
  buff[1] = alarm2_count & 0xff;

  err = tps6586x_writes(tps_dev, RTC_ALARM2_HI, sizeof(buff), buff);
  if (err)
    dev_err(tps_dev, "unable to program alarm2\n");

  return err;
}

static int tps6586x_alarm2_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
  rtc_time_to_tm(saved_alarm2_sec, &alrm->time);
...
/* alarm2 is only used for cold-boot */
static const struct rtc_class_ops tps6586x_alarm2_ops = {
  .read_time = tps6586x_rtc_read_time,
  .set_time = tps6586x_rtc_set_time,
  .set_alarm = tps6586x_alarm2_set_alarm,
  .read_alarm = tps6586x_alarm2_read_alarm,
};
...
static int __devinit tps6586x_rtc_probe(struct platform_device *pdev)
{
    ...
    rtc_device_register("tps6586x-alarm2", &pdev->dev,
                        &tps6586x_alarm2_ops, THIS_MODULE);
