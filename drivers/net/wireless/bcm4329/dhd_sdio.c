...
/* For read Country code from EEPROM */
extern void dmieep_get_wifi_countrycode(char *code);
...
process_nvram_vars(char *varbuf, uint len)
{
	...
	if ((target=strstr(varbuf,"ccode=ALL")) == NULL)
		printk("Can't find ccode in file\n");
	else{
		memset(my_ccode_dmi, 0x0, sizeof(my_ccode_dmi));
		dmieep_get_wifi_countrycode(my_ccode_dmi);
		printk("Country code in DMI is %s\n", my_ccode_dmi);
		if (my_ccode_dmi[0] < 'A' || my_ccode_dmi[0] > 'Z' || my_ccode_dmi[1] < 'A' || my_ccode_dmi[1] > 'Z')
			printk("Country code is invalid, skip\n");
		else{
			memcpy(my_ccode+6, my_ccode_dmi, 2);
			memcpy(target, my_ccode, sizeof(my_ccode)-1);
		}
	}
