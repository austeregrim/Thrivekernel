...
{
    /* printk("tegra_usb_phy_power_off: enter\n"); */
	if (phy_is_ulpi(phy)) {
		struct tegra_ulpi_config *ulpi_config = phy->config;
		if (ulpi_config->inf_type == TEGRA_USB_LINK_ULPI)
	...
	if (phy->regulator_on && 0/* (tegra_get_revision() >= TEGRA_REVISION_A03) */) {
		regulator_disable(phy->reg_vdd);
		phy->regulator_on = 0;
	}
    /* printk("tegra_usb_phy_power_off: exit\n"); */
}
