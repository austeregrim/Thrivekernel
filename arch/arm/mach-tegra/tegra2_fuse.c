...
static void StrToInt(const char *str,
	...
	const char *buf, size_t count)
{
	...
	if (DIV_ROUND_UP(count - 1, 2) > fuse_info_tbl[param].sz) {
	...
	StrToInt(buf, raw_byte_data, count - 1, (fuse_info_tbl[param].nbits + BITS_PER_BYTE - 1)/BITS_PER_BYTE);
...
static void IntToStr(char *arr, char *str, unsigned int MaxSize)
{
	int i = 0, j = 0;
	char tmp[BUF_SIZE]={0};
	strcpy(tmp,"0");
...
static ssize_t fuse_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	ret = tegra_fuse_read(param, (u32*)data, fuse_info_tbl[param].sz);
	if (ret) {
		pr_err("%s: read fail(%d)\n", __func__, ret);
		return ret;
	}

	IntToStr((char*)data, buf, (fuse_info_tbl[param].nbits + BITS_PER_BYTE - 1)/BITS_PER_BYTE);
	return strlen(buf);
}
